      ...
      private static async void ClientMessage(object sender, MessageEventArgs e)
      {
         try {
            ...
            if (message.Text == "/start") {
               ...
               else if (user.Message == "waitinvitecode") {
                  try {
                     await client.EditMessageReplyMarkupAsync(message.Chat.Id, message.MessageId - 1, replyMarkup: null);
                  } catch { }
                  Connect.Query("update `User` set message = 'none' where id = " + message.Chat.Id + ";");
                  var refer = users.Find(x => x.Id.ToString() == message.Text);
                  if (refer != null) {
                     Connect.Query("update `User` set id_invited = " + message.Text + " where id = " + message.Chat.Id + ";");
                     await client.SendTextMessageAsync(message.Chat.Id, "✅ Теперь вы реферал пользователя \"" + message.Text + "\"", replyMarkup: new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Назад", "LocalArea") } }));
                  }
                  else
                     await client.SendTextMessageAsync(message.Chat.Id, "⛔️ Данного пользователя не сущетсвует", replyMarkup: new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Назад", "LocalArea") } }));
               }
            }
            else {
         ...
         try {
            ...
            if (data == "Help") {
               ...
               if (user != null) {
                  ...
                  if (user.Sub == "active") {
                     if (user.Id_invited != 0)
                        await client.SendTextMessageAsync(message.Chat.Id, "*Личный кабинет*\n\nID: " + user.Id + "\nUsername: " + message.From.FirstName + "\nКод пригласившего: " + refer + "\nPremium подписка: " + premium + "\nБаланс: " + user.Balance + " ₽", Telegram.Bot.Types.Enums.ParseMode.Markdown, replyMarkup: Keyboard.areaSubKey);
                     else
                        await client.SendTextMessageAsync(message.Chat.Id, "*Личный кабинет*\n\nID: " + user.Id + "\nUsername: " + message.From.FirstName + "\nКод пригласившего: " + refer + "\nPremium подписка: " + premium + "\nБаланс: " + user.Balance + " ₽", Telegram.Bot.Types.Enums.ParseMode.Markdown, replyMarkup: Keyboard.areaSubRefKey);
                  }
                  else {
                     if (user.Id_invited != 0)
                        await client.SendTextMessageAsync(message.Chat.Id, "*Личный кабинет*\n\nID: " + user.Id + "\nUsername: " + message.From.FirstName + "\nКод пригласившего: " + refer + "\nPremium подписка: " + premium + "\nБаланс: " + user.Balance + " ₽", Telegram.Bot.Types.Enums.ParseMode.Markdown, replyMarkup: Keyboard.areaKey);
                     else
                        await client.SendTextMessageAsync(message.Chat.Id, "*Личный кабинет*\n\nID: " + user.Id + "\nUsername: " + message.From.FirstName + "\nКод пригласившего: " + refer + "\nPremium подписка: " + premium + "\nБаланс: " + user.Balance + " ₽", Telegram.Bot.Types.Enums.ParseMode.Markdown, replyMarkup: Keyboard.areaRefKey);
                  }
               }
            }
            else if (data == "SendRefer") {
               Connect.Query("update `User` set message = 'waitinvitecode' where id = " + message.Chat.Id + ";");
               await client.EditMessageTextAsync(message.Chat.Id, message.MessageId, "Введите ID пригласившего пользователя", replyMarkup: new InlineKeyboardMarkup(new[] { new[] { InlineKeyboardButton.WithCallbackData("Назад", "LocalArea") } }));
            }
